...
* **🛡️ Fault Tolerance:** Auto-reconnect on network drops, retry with exponential backoff, FloodWait handling.
...
* **🖥️ Modern GUI:** Built with Flet (Flutter for Python) with real-time progress bars, statistics, and system logs.
...
* **🛡️ Отказоустойчивость:** Автопереподключение при обрывах сети, повторные попытки с экспоненциальной задержкой, обработка FloodWait.
...
* **🖥️ Современный GUI:** Построен на Flet (Flutter для Python) с прогресс-барами, статистикой и системными логами в реальном времени.
